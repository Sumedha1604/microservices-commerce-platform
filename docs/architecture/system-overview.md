...
The platform combines a React storefront with 12 independently deployable Spring Boot
...
flowchart TB
  ...
  APIClient[Other API client] --> Gateway
...
Every application has a non-root multi-stage image. The local Kustomize overlay deploys the frontend,
12 service Deployments/ClusterIP Services, one PostgreSQL 17 StatefulSet with ten logical
databases/users, one single-node ephemeral KRaft Kafka broker, and Prometheus. Only the frontend and
gateway become LoadBalancers; downstream microservices remain internal.
