 ...
 *
 * <p>Same two-read approach as search-service's parser (a deliberate copy, not a shared library:
 * each consumer validates exactly what it relies on). The first read binds
 * {@code EventEnvelope<JsonNode>} and exposes the raw payload tree; the second binds the typed payload
 * chosen by the {@code eventType} string straight from the text, so the price keeps its exact scale.
 * The tree proves {@code version} and {@code active} were actually sent - both are primitives on the
 * record and would otherwise read a missing value as {@code 0}/{@code false}.
 *
 * <p>Validated here are only the fields recommendation scoring and visibility use: productId,
 * version, name, slug, categoryId, price, currency, status and active. Every rejection is a
